...
POST /api/zones/ (если сделаешь отдельный ViewSet с create)
(или создавай через /admin):
```
{
  "name": "Zone A",
  "capacity": 10,
...
Проверяется capacity, current_load обновляются согласно бизнес-логике.

## WebSocket события

Подключиться к ws://localhost:8000/ws/containers/

При создании / обновлении / назначении контейнера придут сообщения вида:
```
{
  "event": "created",
  "data": {
